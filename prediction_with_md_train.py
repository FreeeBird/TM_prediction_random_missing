...
class Mask_Mae(nn.Module):
    def __init__(self, target='masked'):
        super().__init__()
        self.target = target
    ...
    def forward(self, x, y, mask):
        ...
        if self.target == 'masked':
            ...
            return torch.sum((1-mask) * torch.abs(x - y)) / torch.sum(1-mask)
        

criterion = nn.MSELoss()
res_loss_func = nn.L1Loss()
# res_loss_func = Mask_Mae(target='obs')
# res_loss_func = Mask_Mae(target='masked')
...
def train(model,optimizer,criterion,dataloader,e,writer):
    ...
    for x, y in tqdm(dataloader,ncols=80,position=0):
        ...
        # print(loss2)
        # if e<25:
        #     loss = res_loss_func(x_hat,x) 
        # elif e<50:
        #     loss = criterion(y_hat, y)
        # else:
        loss1 = criterion(y_hat, y)
        # loss2 = res_loss_func(x_hat,x,mask) 
...
if __name__ == '__main__':
    ...
    for i in range(1,args.rounds+1):
        ...
        logger.info("TensorBoard Path:"+tdw_path)
        early_stop = EarlyStopping(patience=args.early_stop, logger=logger)
        # 定义Summary_Writer
        writer = SummaryWriter(tdw_path,flush_secs=20)  
        # dataloader
        dataloader = get_dataloaders(data_path=data_path,train_rate=args.train_rate,
        seq_len=args.seq_len,pre_len=args.pre_len
        ,missing_ratio=args.missing_ratio,missing_index=i,
        ...
        for e in range(1, args.epochs + 1):
            train_loss = train(model,optimizer,criterion,dataloader['train'],e,writer)
            val_loss = validate(model,criterion,dataloader['val'],e,writer)
