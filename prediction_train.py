...
# 
# To train a prediction model with complete data
# 
...
def test(model,dataloader,num_flows,device):
...
if __name__ == '__main__':
    ...
    args.m_adj = np.load(get_adj_matrix(args.dataset))
    # set gpu
    if args.gpu == 1:
    ...
    for i in range(1,args.rounds+1):
        ...
        ,missing_ratio=args.missing_ratio,missing_index=i,batch_size=args.batch_size,
        num_workers=args.cpu,imputer=None,test=False,std=args.std)
        ...
        for e in range(1, args.epochs + 1):
            ...
            if new_high:
                ...
                tmse,trmse,tr2 = test(model,dataloader['test'],num_flows,device)
                save_epoch_test_result(logger,e,args.epochs,tmse,0.,trmse,tr2)
            if es:
                break
        time_end = time.time()
        ...
        mse,rmse,r2 = test(model,dataloader['test'],num_flows,device)
